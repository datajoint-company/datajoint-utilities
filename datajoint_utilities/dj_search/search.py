...
class DJSearch:
    ...
    def __init__(self, db_prefixes=[""], context=None):
        ...
        self.schema_names = set(
            s for s in iter(dj.list_schemas()) if any(p in s for p in db_prefixes)
        )

        tables_definitions = []
        for schema_name in self.schema_names:  # add progress bar
            self.virtual_modules[schema_name] = dj.create_virtual_module(
                schema_name, schema_name
            )
            schema_definition = self.virtual_modules[schema_name].schema.save()
            schema_definition = re.sub(
                r"(@)?schema(.*)\n", rf"\g<1>{schema_name}\g<2>\n", schema_definition
            )[len(schema_name) + 93 :]

            for match in re.finditer(
                r"VirtualModule\('(\w+)', '(\w+)'\)", schema_definition
            ):
    ...
    def search(self, search_str, level=None, method="_do_search"):
        ...
        if level is not None and level not in ("table", "attribute", "comment"):
            raise ValueError(
                'Argument "level" must be ("table", "attribute", "comment")'
            )
        m = DJMatch(
            search_str,
...
class DJMatch:
    ...
    def _do_search(self, level=None):
        for match in re.finditer(
            r" *(class\s\w*?)?({})".format(self.search_str),
            self._definition_string,
            re.I,
        ):
            ...
            if level == "table":
                if not is_class:
                    ...
                    if re.match(
                        r"(\w+)\(", self._definition_string[match.span(2)[-1] :]
                    ):
                        continue
            ...
            line = self._definition_string[
                line_start.span()[0] : line_end.span()[-1] + match.span()[-1]
            ]

            if (
                "dj.VirtualModule" in line
            ...
            class_end = next(
                re.finditer(
                    'definition = """.*?"""' if is_class else '"""',
                    self._definition_string[match.span()[-1] :],
                    re.DOTALL,
                )
            )

            tbl_defi = self._definition_string[
                class_start.span()[0] : class_end.span()[-1] + match.span()[-1]
            ]
            ...
            if tbl_tier == "dj.Part":
                master_prepend = "@{}\nclass {}({}):\n\n\t...\n\n".format(
                    schema_name, master_name, master_tier
                )
                key = "{}.{}.{}".format(schema_name, master_name, tbl_name)
                try:
                    table = getattr(
                        getattr(self._virtual_modules[schema_name], master_name),
                        tbl_name,
                    )
            ...
            color_shift = len(
                re.findall(
                    r"\x1b\[31m{}\x1b\[0m".format(self.search_str), tbl_defi, re.I
                )
            ) * len(colored("", "red"))
            tbl_defi = "".join(
                [
    ...
    def _do_search2(self, level=None):
        ...
        for match_definition in [
            ...
            if (
                i > 1
                and "dj.Schema" not in b
                and next(re.finditer(level_regex, b, re.I), False)
            )
        ]:
            schema_name, table_name, table_tier = (
                re.compile(
                    rf"@(?P<schema_name>{w}){s}[{n}]"
                    rf"{s}class{s}(?P<table_name>{w}){s}\((?P<table_tier>{e})\):{s}(?=[#{n}])"
                )
            ...
            self.matches[f"{schema_name}.{table_name}"] = {
                ...
                "pretty_definition": match_definition,
                "schema_name": schema_name,
                "table": table_name,
                "tier": table_tier,
                "matches": [],
            ...
            for i, match in enumerate(
                [m for m in re.finditer(level_regex, match_definition, re.I)]
            ):
